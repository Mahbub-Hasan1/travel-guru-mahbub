...
const Home = () => {
    ...
    const [travel] = useState(travelData);
    ...
    return (
        <div className="backgroundImage d-flex bd-highlight mb-3 align-items-center">
            <div style={{ width: '130%', color: 'white', margin: '7%' }} className="mr-auto p-2 bd-highlight">
                ...
                <p>{item.TravelBody}</p>
                <Link to={`/DetailsItem/${item.id}`}>
                    <button className="iteBtn">{item.TravelTitle}</button>
                </Link>
            ...
            <div className="ml-auto p-2 bd-highlight travelItem">
                {travel.map(
                    travelItem =>
                        <img key={travelItem.id} className="image"
                            onClick={() => handle(travelItem)}
                            onLoad={() => handle(travelItem)}
                            src={travelItem.img} alt=""
                        />
                )}
            </div>
